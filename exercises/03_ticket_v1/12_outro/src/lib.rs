...
impl Order {
    fn check_product_name(product_name: &String) {
        ...
        if product_name.len() > 300 {
            panic!("product name can't longer than 300")
        }
    }
    fn check_qunatity(quantity: u32) {
        if quantity <= 0 {
            panic!("quantity must greater than 0")
        }
    }
    fn check_unit_price(unit_price: u32) {
        if unit_price <= 0 {
            panic!("unit price must greater than 0")
        }
    }

    pub fn new(product_name: String, quantity: u32, unit_price: u32) -> Order {
        Order::check_product_name(&product_name);
        Order::check_qunatity(quantity);
